...
if [ "$RUN_ON_ALPINE" == "true" ]; then
  docker build -t test-image "alpine-docker"
  docker run -v /var/run/docker.sock:/var/run/docker.sock \
      -e GOPATH="$GOPATH" \
      -v "$BASE_DIR":"$BASE_DIR" \
      -v "$GOPATH":"$GOPATH" test-image \
      go test "${BASE_DIR}/run-system-test/system_test.go" -v -timeout=10m -run TestAgentIntegration -pprof_nodejs_path="$BASE_DIR" -run_only_v8_canary_test="$RUN_ONLY_V8_CANARY_TEST" -binary_host="$BINARY_HOST"
else
